...
<div class="row">
	<div class="col-sm-12">
		<form name="report" class="form-inline" method="POST" action="" autocomplete="off">
...
if($party_code==0){
	$credit_result_total=mysqli_query($con, "SELECT sum(amount) AS credit FROM records WHERE create_date BETWEEN STR_TO_DATE('$from_date','%d/%m/%Y') and STR_TO_DATE('$to_date','%d/%m/%Y') and amount_type='c' and status='e' and particular LIKE '$particular%'");
	$debit_result_total=mysqli_query($con, "SELECT sum(amount) AS debit FROM records WHERE create_date BETWEEN STR_TO_DATE('$from_date','%d/%m/%Y') and STR_TO_DATE('$to_date','%d/%m/%Y') and amount_type='d' and status='e' and particular LIKE '$particular%'");

	$credit_result=mysqli_query($con, "SELECT *,DATE_FORMAT(create_date,'%d/%m/%Y') AS date FROM records,members WHERE records.party_id=members.member_id and create_date BETWEEN STR_TO_DATE('$from_date','%d/%m/%Y') and STR_TO_DATE('$to_date','%d/%m/%Y') and amount_type='c' and status='e' and particular LIKE '$particular%' ORDER BY create_date desc");
	$debit_result=mysqli_query($con, "SELECT *,DATE_FORMAT(create_date,'%d/%m/%Y') AS date FROM records,members WHERE records.party_id=members.member_id and create_date BETWEEN STR_TO_DATE('$from_date','%d/%m/%Y') and STR_TO_DATE('$to_date','%d/%m/%Y') and amount_type='d' and status='e' and particular LIKE '$particular%' ORDER BY create_date desc");

}
else{
	$credit_result_total=mysqli_query($con, "SELECT sum(amount) AS credit FROM records WHERE party_id=$party_code and create_date BETWEEN STR_TO_DATE('$from_date','%d/%m/%Y') and STR_TO_DATE('$to_date','%d/%m/%Y') and amount_type='c' and status='e'");
	$debit_result_total=mysqli_query($con, "SELECT sum(amount) AS debit FROM records WHERE party_id=$party_code and create_date BETWEEN STR_TO_DATE('$from_date','%d/%m/%Y') and STR_TO_DATE('$to_date','%d/%m/%Y') and amount_type='d' and status='e'");

	$credit_result=mysqli_query($con, "SELECT *,DATE_FORMAT(create_date,'%d/%m/%Y') AS date FROM records,members WHERE records.party_id=members.member_id and party_id=$party_code and create_date BETWEEN STR_TO_DATE('$from_date','%d/%m/%Y') and STR_TO_DATE('$to_date','%d/%m/%Y') and amount_type='c' and status='e' and particular LIKE '$particular%' ORDER BY id desc");
	$debit_result=mysqli_query($con, "SELECT *,DATE_FORMAT(create_date,'%d/%m/%Y') AS date FROM records,members WHERE records.party_id=members.member_id and party_id=$party_code and create_date BETWEEN STR_TO_DATE('$from_date','%d/%m/%Y') and STR_TO_DATE('$to_date','%d/%m/%Y') and amount_type='d' and status='e' and particular LIKE '$particular%' ORDER BY id desc");
}
...
<div class="row">
	<div class="col-sm-6">
		<table class="table table-sm table-striped table-hover" id="credit" cellspacing="1px" cellpadding="0px" width="498px" style="overflow:hidden;">
			<tr>
				<th>Date</th>
				<th>Amount</th>
				<th>Party</th>
	...
	<div class="col-sm-6">
		<table class="table table-sm table-striped table-hover" id="debit" cellspacing="1px" cellpadding="0px" width="498px" style="overflow:hidden;">
			<tr>
				<th>Date</th>
				<th>Amount</th>
				<th>Party</th>
